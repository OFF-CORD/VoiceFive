"""VoiceFive, A Py-Cord Cog (Gear) Made to manage temporary voice channels."""
import discord
import asyncio
...
class Control(discord.Cog):
    ...
    temp_voice = discord.SlashCommandGroup(
        name="voice",
        description="To manage temp voice channels on your server",
        guild_only=True,
    ...
    @temp_voice.command(
        name="setup",
        description="To setup temp channels on your server",
        guild_only=True,
    )
    async def setup(self, ctx: discord.ApplicationContext):
        ...
        await db.add_guild(
            ...
            vc_id=channel.id,
            vc_category_id=category.id,
            channel_limit=0,
            channel_bitrate=3,
        )
        return await ctx.respond(
            f"Done setup the temp voice channel, this is the {channel.mention}!"
        )
    ...
    async def on_voice_state_update(
        ...
        ):  # Yes i know this is unuseful but expect anything from discord.
            return
        ...
        if (
            after_channel
            and after_channel.id in map((lambda x: x[1] if x else []), guild_settings)
        ):  # (Fixed) Possible error, the bot cant be able to work if the guild is using more then one config for the temps
            temp_channel = await after_channel.clone(
    ...
    async def on_guild_channel_update(
        ...
        if not before.id in map(
            (lambda x: x[1] if x else []), (await db.get_guild(before.guild.id))
        ):
            return
        if not after.category:
            try:
                embed = Embeds.DeleteWarning()
                await after.send(f"{after.guild.owner.mention}", embed=embed)
            except:
                pass  #                   ~~^~~ smi colon deez nutz
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        if not channel.id in map(
            (lambda x: x[1] if x else []), (await db.get_guild(channel.guild.id))
        ):
            return
    ...
    async def on_guild_remove(self, guild: discord.Guild):
        if not guild.id in map(
            (lambda x: x[0] if x else []), (await db.get_guild(guild.id))
        ):
            return
        return await asyncio.gather(
            *(
                db.remove_guild(guild.id, channel_id)
                for channel_id in map(
                    lambda x: x[1] if x else [], await db.get_guild(guild.id)
                )
            )
        )
