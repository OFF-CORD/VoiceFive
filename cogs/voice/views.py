...
class Views:
    ...
    class UserSelector(discord.ui.View):
        ...
        @discord.ui.select(
            placeholder="Select something to do with this user",
            options=[
                discord.SelectOption(
                    label="Disconnct",
                    description="Disconnect the user from this channel",
                    value="disconnect",
                ),
                discord.SelectOption(
                    label="Mute",
                    description="Mute this user from talking",
                    value="mute",
                ),
                discord.SelectOption(
                    label="Reject",
                    description="Reject this user from joining the channel",
                    value="reject",
                ),
                discord.SelectOption(
                    label="Hide",
                    description="Hide this channel from this user",
                    value="hide",
                ),
            ],
        )
        async def edit_member_callback(
            ...
            if selected == "reject":
                ...
                await interaction.channel.set_permissions(
                    ...
                    overwrite=params,
                    reason="This user has been rejected by the temp owner.",
                )
                msg = await interaction.original_response()
                await interaction.respond(
                    content=f"The Member {self.edit_user.mention} has been rejected.",
                    view=None,
                    delete_after=5,
                )
                if (
            ...
            if selected == "hide":
                ...
                await interaction.channel.set_permissions(
                    target=self.edit_user,
                    overwrite=params,
                    reason="This channel has been hidden from this user by the temp owner.",
                )
                msg = await interaction.original_response()
                return await msg.edit(
                    content=f"The Channel has been hidden for the user {self.edit_user.mention}.",
                    view=None,
                    delete_after=5,
                )
            ...
            if selected == "disconnect":
                await self.edit_user.move_to(None)
                return await interaction.respond(
                    content=f"The Member {self.edit_user.mention} has been disconnected.",
                    view=None,
                    delete_after=5,
                )
            ...
            if selected == "mute":
                ...
                if params.speak or params.speak is None:
                    ...
                    await interaction.channel.set_permissions(
                        target=self.edit_user,
                        overwrite=params,
                        reason="This user has been muted by the temp owner.",
                    )
                    msg = await interaction.original_response()
                    ...
                    return await msg.edit(
                        content=f"The Member {self.edit_user.mention} has been muted.",
                        view=None,
                        delete_after=5,
                    )
                else:
                    ...
                    return await msg.edit(
                        content=f"The Member {self.edit_user.mention} has been unmuted.",
                        view=None,
                        delete_after=5,
                    )
    ...
    class Dropdown(discord.ui.View):
        ...
        @discord.ui.select(
            ...
            options=[
                discord.SelectOption(
                    label="Info",
                    description="Displays Info about this temp voice",
                    value="info",
                ),
                discord.SelectOption(
                    label="Delete",
                    description="To delete this temp voice",
                    value="delete",
                ),
                discord.SelectOption(
                    label="Name",
                    description="To Change this channel Name",
                    value="name",
                ),
                discord.SelectOption(
                    label="Limit",
                    description="To Change this channel users limit",
                    value="limit",
                ),
                discord.SelectOption(
                    label="Slow Mode",
                    description="To Set/Edit this channel users slow-mode/cooldown",
                    value="slow_mode",
                ),
                discord.SelectOption(
                    label="NSFW",
                    description="To Mark this channel as not safe for work (+18)",
                    value="nsfw",
                ),
                discord.SelectOption(
                    label="Lock",
                    description="To Lock this channel with it's users",
                    value="lock",
                ),
                discord.SelectOption(
                    label="Hide",
                    description="To Hide this channel and only users inside will be able to see it",
                    value="hide",
                ),
                discord.SelectOption(
                    label="Bitrate",
                    description="To Change this channel bitrate",
                    value="bitrate",
                ),
                discord.SelectOption(
                    label="Clear",
                    description="To Clear up this channel Messages",
                    value="clear",
                ),
            ],
        )
        async def settings_callback(
            ...
            # interactions will get responded with a modal view

            channel_data = await db.get_channel(
                None, interaction.channel_id, interaction.guild_id
            )
            if selected == "name":
                ...
                if retry:
                    return await interaction.response.send_message(
                        f"Changing Channel Names Have a 10min Cooldown, Try again in `{round(retry, 1)}` seconds.",
                        ephemeral=True,
                    )
                ...
                modal.add_item(
                    discord.ui.InputText(
                        ...
                        placeholder="Please enter the new channel name",
                ...
                async def modal_callback(interaction: discord.Interaction):
                    ...
                    return await interaction.respond(
                        f"Done, Channel Name has been changed to {channel_name}",
                        ephemeral=True,
                        delete_after=5,
                    )
                ...
                modal.add_item(
                    discord.ui.InputText(
                        label="New Channel Limit",
                        placeholder="Please enter the new channel limit (0 for unlimited)",
                ...
                async def modal_callback(interaction: discord.Interaction):
                    ...
                    if not channel_limit.isnumeric():
                        return await interaction.respond(
                            f"Please Input a Valid Number!",
                            ephemeral=True,
                            delete_after=5,
                        )
                    await interaction.channel.edit(user_limit=int(channel_limit))
                    return await interaction.respond(
                        f"Done, Channel User Limit has been changed to {'Unlimited' if int(channel_limit) == 0 else channel_limit}",
                        ephemeral=True,
                        delete_after=5,
                    )
                ...
                async def modal_callback(interaction: discord.Interaction):
                    ...
                    if not modal_value.isnumeric():
                        ...
                        return await interaction.respond(
                            f"Please Input a Valid Number Between 8 and {str(channel_bits)}",
                            ephemeral=True,
                            delete_after=5,
                        )
                    await interaction.channel.edit(bitrate=(int(modal_value) * 1000))
                    return await interaction.respond(
                        f"Done, Channel Bitrate has been Changed To {modal_value}",
                        ephemeral=True,
                        delete_after=5,
                    )
            ...
            if (
                selected == "info"
            ):  # its unnecessary adding an owner check for this.. right?
            ...
            if not interaction.user.id in channel_data:
                return await interaction.respond(
                    "This Option is not available for you, only channel owner",
                    ephemeral=True,
                    delete_after=5,
                )

            if selected == "delete":
                return await interaction.user.move_to(
                    None
                )  # move to None -> user vc channel no.

            elif selected == "slow_mode":
                view = discord.ui.View(timeout=300)
                dropdown = discord.ui.Select(
                    placeholder="Please Select a Cooldown Period",
                    options=[
                ...
                async def slow_mode_select_callback(interaction: discord.Interaction):
                    ...
                    msg = await interaction.original_response()
                    return await msg.edit(
                        f"Done, Slow Mode Has been Changed", view=None, delete_after=5
                    )

                dropdown.callback = slow_mode_select_callback
                return await interaction.respond(
                    "Select Slowmode Delay From This DropDown",
                    view=view,
                    ephemeral=True,
                    delete_after=300,
                )
                ...
                if interaction.channel.nsfw:
                    ...
                    return await interaction.respond(
                        f"Done, Channel has been unMarked as NSFW",
                        ephemeral=True,
                        delete_after=5,
                    )
                else:
                    ...
                    return await interaction.respond(
                        f"Done, Channel has been Marked as NSFW",
                        ephemeral=True,
                        delete_after=5,
                    )
                ...
                # changes will get reflected on these objects after a bot restart because caching, kinda bad but this is necessary to not fuck your memory
                if everyone_perms.connect:
                    ...
                    return await interaction.respond(
                        f"Done, Channel has been locked successfully",
                        ephemeral=True,
                        delete_after=5,
                    )
                else:
                    ...
                    return await interaction.respond(
                        f"Done, Reverted to locked", ephemeral=True, delete_after=5
                    )
                ...
                if everyone_perms.view_channel:
                    ...
                    return await interaction.respond(
                        f"Done, Channel has been hidden successfully",
                        ephemeral=True,
                        delete_after=5,
                    )
                else:
                    ...
                    return await interaction.respond(
                        f"Done, Reverted to unhidden", ephemeral=True, delete_after=5
                    )
                ...
                return await interaction.respond(
                    f"Are you sure you want to continue?", ephemeral=True, view=btn_view
                )
        ...
        async def view_callback(
            ...
            if await db.get_channel(
                interaction.user.id, interaction.channel_id, interaction.guild_id
            ):
                ...
                if not selected_user.guild:
                    ...
                    return await interaction.respond(
                        f'Dear {interaction.user.mention}, if you have that thing called "Eyes ✨" you will be able to see that user is a bot!',  # better way?
                        ephemeral=True,
                        delete_after=5,
                    )
                await interaction.respond(
                    "Select an option to edit this user.",
                ...
                return await interaction.respond(
                    "You are not this channel owner.", ephemeral=True, delete_after=5
                )


# typically this code was written without an internet connection, all the thank to egypt.
